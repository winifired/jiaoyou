	...
	<view class="writeMsg">
		<form @submit="formSubmit">
			...
			<view class="flex area-between item" @click="chooseAvatar">
				...
				<image :src="form.headImg?form.headImg:'../../static/image/avatar.png'" mode="aspectFill" class="avatar"></image>
			</view>
			...
			<view class="flex area-between item">
				<view class="name color818 font32">性别 <text class="color999 font26 ml-22">注：选择后不可修改</text></view>
			</view>
			...
			<view class="flex item tags">
				...
				<view class="tag color818 font28" v-for="(item,index) in objectdisposition" :key="item.id"
					:class="item.checked?'choosedLabel':''" @click="toggleLabel(index,objectdisposition)">
					{{item.labelName}}
				</view>
			</view>
			<button form-type="submit" class="confirm next">下一步</button>
		</form>
		...
		methods: {
			...
			formSubmit() {
				...
				for(let i in this.form){
					if(i!='coverImg'&&i!='matchImg'&&i!='targetHobby'&&i!='targetCharacter'&&!this.form[i]){
						this.$toast('请填写完整的信息');
						return;
					}
				}
				if(!this.form.targetHobby&&!this.form.targetCharacter){
					this.$toast('请选择期望对象');
					return;
				}
				uni.navigateTo({
					url: '/pages/login/writeMsgImg?form=' + JSON.stringify(this.form)
				})
			},
			bindDateChange(e) {
				// 生日
